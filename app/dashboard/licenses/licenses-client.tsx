...
import { BadgeCheck, Ban, Copy, KeyRound, Loader2, RotateCcw, ShieldOff, Trash2, Users } from 'lucide-react'
...
export function LicensesClient({
  ...
  async function createLicense(formData: FormData) {
    ...
    startTransition(async () => {
      ...
      try {
        ...
        const listResponse = await fetch(`/api/licenses?script_id=${encodeURIComponent(selectedScriptId)}`)
        if (listResponse.ok) {
  ...
  async function toggleAssignments(licenseId: string) {
    ...
    setAssignmentsByLicense((state) => ({
      ...state,
      [licenseId]: { loading: true, items: [], error: null },
    }))
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setAssignmentsByLicense((state) => ({
          ...state,
          [licenseId]: { loading: false, items: [], error: message },
        }))
        return
      }
      ...
      setAssignmentsByLicense((state) => ({
        ...state,
        [licenseId]: { loading: false, items: body.assignments ?? [], error: null },
      }))
    } catch {
      setAssignmentsByLicense((state) => ({
        ...state,
        [licenseId]: { loading: false, items: [], error: 'Failed to load assignments' },
      }))
    }
  }
            ...
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/50">
              <div className="flex items-center justify-between border-b border-zinc-800 px-5 py-4">
                <div>
                  ...
                  <p className="text-sm text-zinc-400">{licenses.length} license{licenses.length === 1 ? '' : 's'}</p>
                </div>
              ...
              {licenses.length === 0 ? (
                <div className="p-5">
                  <EmptyState title="No licenses yet" description="Create a license to start testing premium distribution." />
                </div>
              ) : (
                <div className="divide-y divide-zinc-800">
                  {licenses.map((license) => {
                    ...
                    return (
                      <article key={license.id} className="p-5">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                          <div className="min-w-0 space-y-3">
                            <div className="flex flex-wrap items-center gap-2">
                              <StatusBadge status={license.status} />
                              <span className="font-mono text-xs text-zinc-500">{license.id}</span>
                            </div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            <button type="button" onClick={() => toggleAssignments(license.id)} className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-800 px-3 py-2 text-xs text-zinc-300 hover:bg-zinc-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-600">
                              <Users className="h-3.5 w-3.5" aria-hidden="true" /> {assignments ? 'Hide' : 'View'} Assignments
                            </button>
                          </div>
                        </div>
                        ...
                        {assignments && (
                          <div className="mt-4 rounded-lg border border-zinc-800 bg-zinc-950 p-4">
                            {assignments.loading ? (
                              <div className="flex items-center gap-2 text-sm text-zinc-400"><Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /> Loading assignments</div>
                            ) : assignments.error ? (
